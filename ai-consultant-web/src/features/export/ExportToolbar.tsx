...
function formatGxxMarkdown(data: any) {
  return [
    ...
    '',
    '## 高小新评分雷达',
    `- 高 (High)：${data.scores?.high ?? 0} / 5${data.scoreReasons?.high ? ` — ${data.scoreReasons.high}` : ''}`,
    `- 小 (Small)：${data.scores?.small ?? 0} / 5${data.scoreReasons?.small ? ` — ${data.scoreReasons.small}` : ''}`,
    `- 新 (New)：${data.scores?.new ?? 0} / 5${data.scoreReasons?.new ? ` — ${data.scoreReasons.new}` : ''}`,
    '',
    '## 诊断总结',
